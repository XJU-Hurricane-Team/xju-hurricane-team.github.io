...
**结构体的实质就是自定义数据类型**，因此在定义结构体变量前需要先对结构体进行声明（也就是自定义结构体内应该有什么的过程），在这之后你可以像上面例子里一样在声明结构体的时候声明结构体变量，也可以用以下方法

```c
struct student student1, student2;		//在结构体声明之后进行，struct是关键字
```

或者通过给结构体类型起别名的方式来简化

```c
typedef struct student				//typedef给struct student变量类型起了一个Student的别名
{
    int num;						
    char name[20];					
    char sex;							
...
Student student1, student2;			//用Student来表示变量类型
```

结构体变量赋初值可以在声明的时候用大括号依次赋值，括号内的数据会根据结构体声明时成员的排列顺序来依次给成员赋值。

```c
Student student1 = {89031, "Li Lin", 'M', "123 Beijing Road"};//student1的num变量的值就为89031
```

或者通过访问结构体成员来赋值，一般通过 **变量名.成员名** 来访问成员，如果是结构体指针也可以使用 **指针名->成员名** 或者 **(*指针名).成员名** 的访问方式。

**ps.你无法一次性输出整个结构体变量，只能输出结构体变量的成员。**
...
结构体同样也可以作为函数参数，一般有3个方法：

1. 用结构体变量的成员作参数
2. 用结构体变量作参数。老版本的C系统不允许用结构体变量作实参，ANSI C取消了这一限制。但是如果在执行被调用函数期间改变了形参（也是结构体变量）的值，该值不能返回主调函数，这往往造成使用上的不便。因此一般较少用这种方法。
3. 用指向结构体变量（或数组）的指针作实参，将结构体变量（或数组）的地址传给形参。



结构体是在一块连续的内存空间中存储的，但是因为[字节对齐](https://zhuanlan.zhihu.com/p/30007037)所以在这块连续的内存空间中每一个结构体成员并不是紧密挨在一起的，

他们的排列方式遵顼以下规则：

-  结构体变量的首地址能够被其最宽基本类型成员的大小所整除；
-  结构体每个成员相对于结构体首地址的偏移量（offset）都是该成员大小的整数倍的最小值，如有需要编译器会在成员之间加上填充空白字节（internal adding）；
-  结构体的总大小为结构体最宽基本类型成员大小的整数倍，如有需要编译器会在最末一个成员之后加上填充字节（trailing padding）

在这个规则下成员完全相同但成员间摆列顺序不同的两个结构体所占内存空间就不相同，例如：
...
详情可看->[浅谈Struct(结构体大小如何确定以及内部成员如何存放)_怎么判断结构体成员大小-CSDN博客](https://blog.csdn.net/qq_36534111/article/details/115273413)
