...
而在另一种情况下，运行调试程序的 PC 并不直接与嵌入式设备物理连接，而是通过网络与直接连接调试器的 PC 交互，这一交互是通过本地 PC 运行 J-LinkGDBServer 程序（充当GDBServer的角色）和远程 PC 运行 J-LinkRemoteServer 程序实现的。不过在GDB看来上述两种调试情况并没有区别，因为它仅与 GDBServer 交互。

![Jlink工作模式](./Picture/Jlink工作模式.png)

### [J-Link Commander ](https://kb.segger.com/J-Link_Commander)

J-Link 的一款命令行工具，它能够进行简单的调试，如停止、步进、走、读寄存器等，可以验证目标连接、擦除芯片、调整J-Link设置。

### [J-Link GDB Server ](https://kb.segger.com/J-Link_GDB_Server)

J-LinkGDBServer 是 GDB 的远程服务器，使 GDB 能够通过 J-Link 连接并与目标设备通信。 J-LinkGDBServer 与 GDB 通过 TCP/IP 连接通信，使用标准的 GDB 远程协议。通过 J-LinkGDBServer，可以在目标设备的 ROM 和 Flash 中调试，具有无限断点功能，同时还支持一些 GDB 未直接实现的功能，但可通过 monitor 命令访问，这些命令可直接通过 GDB 发送。
...
### [J-Flash](https://kb.segger.com/UM08003_JFlash)

J-Flash 是一款独立的闪存编程软件，能够通过 J-Link 高速读写芯片的 Flash。

### [J-LinkRemoteServer](https://www.segger.com/products/debug-probes/j-link/tools/j-link-remote-server/)

J-LinkRemoteServer 用于网络调试。
