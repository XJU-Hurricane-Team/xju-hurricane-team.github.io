...
1. https://github.com/micro-ROS/micro_ros_setup?tab=readme-ov-file
2. https://micro.ros.org/docs/tutorials/advanced/create_new_type/
...
2. 后续步骤参考官方教程（很详细）：https://micro.ros.org/docs/tutorials/advanced/create_new_type/



### 生成静态库

  生成静态库主要是依据命令`ros2 run micro_ros_setup build_firmware.sh $(pwd)/my_custom_toolchain.cmake $(pwd)/my_custom_colcon.meta`

  所以我们只需准备好my_custom_toolchain.cmake和my_custom_colcon.meta
...
* **my_custom_colcon.meta：**作用是处理中间件配置与裁剪。microros的功能很多，有些不需要的功能我们可以在此禁止掉，减轻单片机的运行压力。
  ...
  准备好上述两个文件，运行`ros2 run micro_ros_setup build_firmware.sh $(pwd)/my_custom_toolchain.cmake $(pwd)/my_custom_colcon.meta`即可构建生成静态库，得到文件如下
